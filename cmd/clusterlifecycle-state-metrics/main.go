...
import (
	...
	"github.com/stolostron/clusterlifecycle-state-metrics/pkg/tlsprofile"
...
func start(opts *options.Options) {
	...
	if err := tlsprofile.StartTLSProfileWatcher(ctx, config, cancel); err != nil {
		klog.Warningf("Failed to start TLS profile watcher: %v", err)
	}
	...
	go telemetryServer(ctx, ocmMetricsRegistry, config, opts.TelemetryHost, opts.HTTPTelemetryPort, opts.HTTPSTelemetryPort, opts.TLSCrtFile, opts.TLSKeyFile)
...
func telemetryServer(
	...
	if tlsCrtFile != "" && tlsKeyFile != "" {
		...
		if err != nil {
			klog.Warningf("Failed to get TLS config, using default: %v", err)
