...
func GetTLSConfig(kubeConfig *rest.Config) (*tls.Config, error) {
	...
	if err != nil {
		klog.Warningf("Failed to get TLS security profile from cluster, using default TLS 1.2: %v", err)
...
func GetTLSSecurityProfile(kubeConfig *rest.Config) (*configv1.TLSSecurityProfile, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			klog.Info("APIServer CR not found, not running on OpenShift cluster")
			return nil, err
		}
