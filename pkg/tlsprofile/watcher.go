...
func StartTLSProfileWatcher(ctx context.Context, kubeConfig *rest.Config, cancel context.CancelFunc) error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			klog.Info("APIServer CR not found, not running on OpenShift - TLS profile watcher disabled")
			return nil
		}
		klog.Warningf("Failed to get initial TLS profile, watcher will not start: %v", err)
	...
	if err != nil {
		klog.Warningf("Failed to create config client for watcher: %v", err)
		return nil
	}
